...
# Clear disk:
  zerombr
  clearpart --all --initlabel --drives=_TYPE_DISK_

# Create partitions on disk:
# investigate putting this into LVM
  part /boot --size=256 --fstype=ext3 --ondisk=_TYPE_DISK_
  part pv.01 --size=100 --grow --ondisk=_TYPE_DISK_

# Create VG on partition:
  volgroup rootvg._HOSTNAME_OPTION_ pv.01 --pesize=32768

# Create LV's on VG:
  logvol /                --fstype=ext3 --name=rootlv    --vgname=rootvg._HOSTNAME_OPTION_ --size=8192
  logvol swap             --fstype=swap --name=swap01lv  --vgname=rootvg._HOSTNAME_OPTION_ --size=2048
  logvol /home            --fstype=ext3 --name=homelv    --vgname=rootvg._HOSTNAME_OPTION_ --size=6144

# Deal with bootloader:
  bootloader grub --append="_TYPE_BOOT_"
#----------------------------------------------------------------------
...
ssh-keygen -q -f /root/.ssh/id_rsa -t rsa -N "" -C "temp@_HOSTNAME_OPTION_"
